...
function filtrarImagem(req, file, cb) {
  if (!allowedMimeTypes.includes(file.mimetype)) {
    return cb(new AppError("Tipo de imagem nao suportada!", 400), false);
  }

  return cb(null, true);
}

function filtrarManual(req, file, cb) {
...
async function imagemProcessada(req, res, next) {
  if (!req.file) return next(new AppError("Imagem nao encontrada!", 400));
  ...
  try {
    ...
    return next(new AppError("Arquivo de imagem invalida!", 400));
  }
}
